...
import it.nowicki.jaroslaw.socialnetwork.backend.domain.exception.CommentNotFoundException
...
import it.nowicki.jaroslaw.socialnetwork.backend.domain.post.PostRepository
...
constructor(private val commentRepository: CommentRepository, private val postRepository: PostRepository, private val userRepository: UserRepository) {
    ...
    fun update(userId: String, commentId: Long?, com: Comment) {
        ...
        if (comment?.user?.userId != user?.userId)
            throw it.nowicki.jaroslaw.socialnetwork.backend.domain.exception.AccessDeniedException()
